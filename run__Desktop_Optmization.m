...
NoEvaluations = 0;
maxSimulationTime = 0.01; % in hours.
...
Nrep =5;
N_sims = 5000;    % number of repetitions.
optimizationMethod = 3; %  1 = Multi-onjective Optimization. 2 = GA. 3 = Pattern Search.
conditionNoSkip =0; % this variable avoid skiping evalutions of HT or NaAs
...
%%
PopulationSize = 14; generations = 5;
EvaluationCondition1 = round((PopulationSize *generations)/1.2);
EvaluationCondition2 = round((PopulationSize *generations)/1.1);
bestFitValue = [Inf,Inf,Inf,Inf];
randInit = 0; % Using Random initial condition
[initPop] = initialPopulation (Model_type);
if randInit ==1
    nParameters = length(initPop);
    randSign =  2*randi([0 1],[1,nParameters])-1;
    variability =0.2;% percentage of variability in intial popultion
...
elseif Model_type == 11  || Model_type == 12 % k13=k24;
    k_34_min =0; k_34_max =0;
    k_43_min =0; k_43_max =0;
    k_24_min =0; k_24_max =0;
    k_42_min =1e-5; k_42_max =0.01;
    
elseif Model_type == 13 || Model_type == 14  % k31= k42;
    k_34_min =0; k_34_max =0;
    k_43_min =0; k_43_max =0;
    k_24_min =1e-5; k_24_max =0.01;
    k_42_min =0; k_42_max =0;
...
if Model_type ==2 || Model_type ==4 || Model_type ==6 || Model_type ==8 || Model_type ==10 || Model_type ==12 || Model_type ==14
    k_crossover_min =1e-5; k_crossover_max=1;
...
elseif optimizationMethod ==2
    optionsGA = optimoptions('ga','MaxGenerations',generations,'PopulationSize',PopulationSize,'ConstraintTolerance',1e-9,'InitialPopulationMatrix',initialMatrix,'Display', 'iter');
    [x,fval] = ga(fitFnc,nvars,[],[],[],[],lb,ub,[],[],optionsGA);
elseif optimizationMethod ==3
    options_PS = optimoptions('patternsearch','Display','iter','MaxIterations',1000,'MeshTolerance',1e-9,'MaxTime',3600*maxSimulationTime);
...
if Model_type == 11  || Model_type == 12 % k13=k24;
    x(:,8) = x(:,4);  % k_34 = k_12;
    x(:,9) = x(:,5);  % k_43 = k_21;
    x(:,10) = x(:,6); % k_24 = k_13;
end

if Model_type == 13 || Model_type == 14 % k31= k42;
    x(:,8) = x(:,4);  % k_34 = k_12;
    x(:,9) = x(:,5);  % k_43 = k_21;
    x(:,11) = x(:,7);  % k_42 = k_31;
end
