function compareDTT_3int_sweep(param, value_sweep,folderName, strExperimentalData,sim_CAP_ONLY, sim_IRES_ONLY, sim_CI,error_CAP_ONLY, error_IRES_ONLY,error_CI,plottingCondition)
pre_namePlot ='DTT_sweep_';
%% Intensity Deffinition
data_1G = strExperimentalData.mean_exp_DTT_CAP;
data_2G = strExperimentalData.mean_exp_DTT_IRES;
data_BG = strExperimentalData.mean_exp_DTT_CAP_IRES;
err_data_1G = strExperimentalData.std_exp_DTT_CAP;
err_data_2G = strExperimentalData.std_exp_DTT_IRES;
err_data_BG = strExperimentalData.std_exp_DTT_CAP_IRES;
ds_expTime = strExperimentalData.timeExp_DTT;
...
if plottingCondition ==1
    ...
    if SupplementaryFigure ==1
        ...
        pre_fname = 'data_Figure_S6_E_DTT_';
    else
        pre_fname = 'data_Figure_5_F_DTT_';    
    end
    ...
    function makeplot (geneType,value_sweep,time,sim,error_sim,data,err_data,color_vector,color_plot, width,height,scaleFig,font_gca,font_labels,font_legend,BoxLineWidth,y_lable,textSize)
        ...
        ylim([-0.05 2]);
        for i =1: size(sim,1)
            text(time(end-8),max([0.07,sim(i,end-8)]),[num2str(value_sweep(i)),'%'],'FontSize',textSize)
        end
