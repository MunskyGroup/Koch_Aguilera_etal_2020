function [sim_CAP_ONLY, sim_IRES_ONLY, sim_CI, error_CAP_ONLY, error_IRES_ONLY,error_CI] = intesities_selection_3int(intensityVector_1G, intensityVector_2G,pointOfNormalization)
...
int_BGm = zeros (nRepetitions,nTimePoints);

memoryPoint =10; % point that checks n points before the current point to consider it as G and B. It keeps memory of the proeccess.
...
for i =1: nRepetitions
    for j =1: nTimePoints
        ...
        if j >memoryPoint
            if intensityVector_1G(i,j) >0 && sum(intensityVector_2G(i,j-memoryPoint:j)) > 0 %%  && intensityVector_2G(i,j) >0 %
                int_BGm (i,j) = intensityVector_1G(i,j);
            end
        else
            if intensityVector_1G(i,j) >0 && sum(intensityVector_2G(i,1:j)) > 0 %%  && intensityVector_2G(i,j) >0 %
                int_BGm (i,j) = intensityVector_1G(i,j);
            end
...
nb = mean(mean(int_BG(:,1:pointOfNormalization)));
...
parfor kk =1: nbootstr
    ...
    int_BGm_C{kk} = zeros (nSelection,nTimePoints);
    for i =1: nSelection
        for j =1: nTimePoints
...
%             if intensityVector_1G_bs(i,j) >0 && sum(intensityVector_2G_bs(i,1:j)) > 0
%                 int_BGm_C{kk} (i,j) = intensityVector_1G_bs(i,j);
%             end
        if j >memoryPoint
            if intensityVector_1G(i,j) >0 && sum(intensityVector_2G_bs(i,j-memoryPoint:j)) > 0 %%  && intensityVector_2G(i,j) >0 %
                int_BGm_C{kk}(i,j) = intensityVector_1G_bs(i,j);
            end
        else
            if intensityVector_1G(i,j) >0 && sum(intensityVector_2G_bs(i,1:j)) > 0 %%  && intensityVector_2G(i,j) >0 %
                int_BGm_C{kk}(i,j) = intensityVector_1G_bs(i,j);
            end
