...
sequenceParameters.ratio_ke_ires = sequenceAnalyzer('Sequence_IRES_Kif18b.txt'); % this is a vector containing the ratio between the specific tRNA_copyNumber (i)/ mean_tRNACopyNumber for each codon
sequenceParameters.ratio_ke_cap = sequenceAnalyzer('Sequence_CAP_KDM5B.txt'); % this is a vector containing the ratio between the specific tRNA_copyNumber (i)/ mean_tRNACopyNumber for each codon
...
sequenceParameters.codon_time_CAP = sum(1./sequenceParameters.ratio_ke_cap);
sequenceParameters.codon_time_IRES = sum(1./sequenceParameters.ratio_ke_ires);
end
