...
if selectedModel == N_Models || justPlot ==1
    ...
    L_op_sel_model_normalized= [sum_LL_wo_Stress_normalized(sel_model),sum_SEM_LL_wo_stress(sel_model)]
    L_op_complex_model_normalized= [sum_LL_wo_Stress_normalized(mostComplexModel),sum_SEM_LL_wo_stress(mostComplexModel)]
    
    %% Reproted values for NaAS
    %     Best_crossvalidation=1; % 100% inibition or k_c =0
    [~, Best_crossvalidation] =min(mean_prediction_Na_1([1:3],sel_model));
    
    L_NA_typeI =[mean_prediction_Na_1(Best_crossvalidation,sel_model)+minVal_Na, sem_prediction_Na_1(Best_crossvalidation,sel_model)]
    
    %    Best_crossvalidation=2; % 66% inibition or k_c =33
    [~, Best_crossvalidation] =min(mean_prediction_Na_2([1:3],sel_model));
    L_NA_typeII =[mean_prediction_Na_2(Best_crossvalidation,sel_model)+minVal_Na,sem_prediction_Na_2(Best_crossvalidation,sel_model)]
    
    % Reproted values for DTT
    % Best_crossvalidation=1; % 100% inibition or k_c =0
    [~, Best_crossvalidation] =min(mean_prediction_DTT_1([1:3],sel_model));
    L_DTT_typeI = [mean_prediction_DTT_1(Best_crossvalidation,sel_model)+minVal_DTT,sem_prediction_DTT_1(Best_crossvalidation,sel_model)]
    
    %     Best_crossvalidation=2; % 66% inibition or k_c =33
    [~, Best_crossvalidation] =min(mean_prediction_DTT_2([1:3],sel_model));
    L_DTT_typeII = [mean_prediction_DTT_2(Best_crossvalidation,sel_model)+minVal_DTT,sem_prediction_DTT_2(Best_crossvalidation,sel_model)]
